(function($) {
    ...
    $(document).on('click', '.fs-option', function() {
        ...
        if ($wrap.hasClass('multiple')) {
            ...
            if($wrap.find('.fs-option').length==$wrap.find('.selected').length){
                $wrap.closest('.fs-wrap').next('.unselected').each(function(i,el){
    ...
    $(document).on('click', '.unselected', function() {
        ...
        $selecteddiv.find('.fs-option').each(function(i,el){
                $(el).addClass('selected');
            });
        var labelText = [];
        var overflowText ='{n} selected';
        $selecteddiv.find('.fs-option.selected').each(function(i, el) {
            labelText.push($(el).find('.fs-option-label').text());
        });
        ...
        else if (labelText.length > 1) {
            labelText = overflowText.replace('{n}', labelText.length);
        }
        else {
            labelText = labelText.join(', ');
        }
        $selecteddiv.find('.fs-label').html(labelText);
       }
       else{
        $selecteddiv.find('.fs-option').each(function(i,el){
                $(el).removeClass('selected');
            });
        $selecteddiv.find('.fs-label').html('Select options');
       }
    });
