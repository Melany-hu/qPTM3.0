...
const BIO_KEYWORDS_EN =
  /\b(protein|gene|kinase|phospho|ptm|post-translational|ubiquitin|acetyl|methyl|site|uniprot|pathway|cell|organism|mutation|disease|cancer|enzyme|peptide|amino\s+acid|dna|rna|chromosome|apoptosis|receptor|antibody|histone|metabolism|signaling|signalling)\b/i;
...
export function classifyQueryMode(message: string, entities: ParsedEntities): QueryMode {
  ...
  if (/collect|采集|pmid.*抽取|literature collection/i.test(m) && entities.pmid) return "collection";
