...
export function parseEntities(message) {
    ...
    const named = message.match(/\b(?:Ser|Thr|Tyr|Lys|Arg|丝氨酸|苏氨酸|酪氨酸)\s*[- ]?(\d{2,5})\b/i);
    if (sty) {
    ...
    if (!out.gene && /\btrp53\b/i.test(message))
        out.gene = "Trp53";
    ...
    if (/mouse|小鼠|mus\s*musculus/i.test(message))
        out.organism = "mouse";
    else if (/human|人|智人|homo\s*sapiens/i.test(message))
        out.organism = "human";
    return out;
}
...
export function extractPositionFromText(text) {
    ...
    if (mouseSite)
        return Number(mouseSite[2]);
    const humanSite = text.match(/\b([STYKR])(\d{2,5})\s*[（(]?\s*人/i);
    if (humanSite)
        return Number(humanSite[2]);
...
export function normalizeTargetIdentity(memory, contextText = "") {
    const blob = `${contextText}\n${memory.gene || ""}\n${memory.organism || ""}`;
    if (/mouse|小鼠/i.test(blob))
