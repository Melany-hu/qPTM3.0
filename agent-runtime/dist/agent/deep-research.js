...
import { addFinding, mergeEntities, memoryPromptBlock, normalizeTargetIdentity, parseEntities, } from "../context/memory.js";
import { callQptmTool, biomcpGetArticle, readQptmResource, searchLiteratureArticles, webSearch, } from "../mcp/hub.js";
...
Max 6 steps. Never use WHO/WHEN/WHERE/WHY labels in title/rationale.`;
    ...
    push(lang === "zh" ? "位点与蛋白背景" : "Site & protein context", "site", lang === "zh" ? "确认靶点与基础注释" : "Confirm target and baseline annotation", tools.filter((t) => /qptm_search|uniprot/i.test(t)).slice(0, 2));
    if (/kinase|上游|调控|who|enzyme|e3|writer/i.test(q) || !/only|仅/.test(q)) {
        push(lang === "zh" ? "上游调控" : "Upstream regulation", "kinase", lang === "zh" ? "检索激酶/酶/调控证据" : "Retrieve kinase/enzyme/regulator evidence", tools.filter((t) => /kinase|enzyme|psp_kinase|gps|ekpi_kinase/i.test(t)).slice(0, 3));
    }
    if (/condition|定量|动力学|fold|when|刺激|处理/i.test(q)) {
        push(lang === "zh" ? "定量与条件" : "Quantitation & conditions", "condition", lang === "zh" ? "检索条件定量数据" : "Retrieve quantitative condition data", tools.filter((t) => /condition|quantitative|ekpi_quant/i.test(t)).slice(0, 3));
    }
    if (/local|定位|domain|where|结构域/i.test(q)) {
        push(lang === "zh" ? "定位与结构域" : "Localization & domains", "localization", lang === "zh" ? "检索定位/结构域背景" : "Retrieve localization/domain context", tools.filter((t) => /local|compartment|domain|interpro|inuloc/i.test(t)).slice(0, 3));
    }
    if (/function|disease|疾病|功能|稳定|why|突变|mutat|drug|药/i.test(q) || steps.length < 2) {
        push(lang === "zh" ? "功能与疾病关联" : "Function & disease", "function", lang === "zh" ? "检索功能、疾病与稳定性证据" : "Retrieve function/disease/stability evidence", tools.filter((t) => /regulatory|disease|stability|funcscore|ptmd|drug|decryptm/i.test(t)).slice(0, 4));
    }
    if (!steps.length) {
        push(lang === "zh" ? "综合检索" : "Broad retrieval", "site", lang === "zh" ? "按可用工具综合检索" : "Broad tool sweep", tools.slice(0, 5));
    }
